...
BUNDLE_ID="com.hannojacobs.NOTR"
SCHEME_NAME="NOTR"
WORKSPACE_PATH="$SCRIPT_DIR/.swiftpm/xcode/package.xcworkspace"
SOURCE_PLIST="$SCRIPT_DIR/Sources/NOTR/Info.plist"
CHANGELOG_PATH="$SCRIPT_DIR/CHANGELOG.md"
APP_ENTITLEMENTS_PATH="$SCRIPT_DIR/NOTR.entitlements"
...
LATEST_LOG_PATH="$HOME/Library/Application Support/NOTR/Logs/latest.log"

RELEASE_CONFIG_FILE="${NOTR_RELEASE_CONFIG:-$SCRIPT_DIR/release.env}"
if [ -f "$RELEASE_CONFIG_FILE" ]; then
    # shellcheck disable=SC1090
    source "$RELEASE_CONFIG_FILE"
fi

NOTR_CODESIGN_IDENTITY="${NOTR_CODESIGN_IDENTITY:-}"
NOTR_CODESIGN_MODE="${NOTR_CODESIGN_MODE:-adhoc}"
NOTR_SPCTL_EXPECT="${NOTR_SPCTL_EXPECT:-}"
...
assert_release_config() {
    case "$NOTR_CODESIGN_MODE" in
        adhoc|developer_id)
            ;;
        *)
            fail "Unsupported NOTR_CODESIGN_MODE value: $NOTR_CODESIGN_MODE"
            ;;
    esac

    if [ -z "$NOTR_SPCTL_EXPECT" ]; then
        if [ "$NOTR_CODESIGN_MODE" = "adhoc" ]; then
            NOTR_SPCTL_EXPECT="rejected"
        else
            NOTR_SPCTL_EXPECT="accepted"
        fi
    fi

    case "$NOTR_SPCTL_EXPECT" in
        accepted|rejected|skip)
            ;;
        *)
            fail "Unsupported NOTR_SPCTL_EXPECT value: $NOTR_SPCTL_EXPECT"
            ;;
    esac
}

codesign_identity_label() {
    if [ "$NOTR_CODESIGN_MODE" = "developer_id" ]; then
        [ -n "$NOTR_CODESIGN_IDENTITY" ] || fail "NOTR_CODESIGN_IDENTITY is required for developer_id mode."
        printf '%s' "$NOTR_CODESIGN_IDENTITY"
    else
        printf '%s' "-"
    fi
}
